/* eslint-disable global-require */
// import assetsImages from '../../../../form-assets/images';
const images = {
  SideMenuIcon: require('./files/_0041_Rectangle-228-Copy.png'),
  PremiumIcon: require('./files/premiumIcon.png'),
  AppIcon: require('./files/app_icon.png'),
  //notProtected: require('./files/notProtected.png'),
  goldOnIcon: require('./files/shield-gold-on.png'),
  goldOffIcon: require('./files/shield-gold-off.png'),

  silverOnIcon: require('./files/shield-silver-on.png'),
  silverOffIcon: require('./files/shield-silver-off.png'),

  platinumOnIcon: require('./files/shield-platinum-on.png'),
  platinumOffIcon: require('./files/shield-platinum-off.png'),

  fmcIcon: require('./files/shield-fmc.png'),
  dashBoardIcon: require('./files/dashboard-logo.png'),

  goldCoin: require('./files/gold-coin.png'),
  silverCoin: require('./files/silver-coin.png'),
  // welcomeBg: assetsImages.welcomeBg,
  // welcomeLogo: assetsImages.welcomeLogo,
  // listDefault: assetsImages.listDefault,
  // logo: assetsImages.logo,
  // appLogo: assetsImages.appLogo,
  // gif: assetsImages.gif,
  // back: require("./files/back.png"),
  // backWhite: require("./files/back-white.png"),
  // search: require("./files/search.png"),
  // facebookIcon: require("./files/facebook.png"),
  // arrowRight: require("./files/arrowRight.png"),
  // profilePhoto: require("./files/user-default.png"),
  // close: require("./files/close.png"),
  // verified: require("./files/verified.png"),
  // back: require("./files/back.png"),
  // enFlag: require("./files/flags_images_us.png"),
  // deFlag: require("./files/flags_images_de.png"),
  // plFlag: require("./files/flags_images_pl.png"),
  // checkbox: assetsImages.checkbox,
  // radio: assetsImages.radio,
  // amazonVouchar: require("./files/amazon-vouchar.png"),
  // markerIcon: assetsImages.markerIcon
};

export default images;

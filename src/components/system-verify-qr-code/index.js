...
let tmpSys = [];
let self;
class SystemVerifyWithQRCode extends Component {
  state = {
    qrCode: true,
    item: null,
  };
  ...
  componentDidMount() {
    self = this;
    Events.on('qrCode', 'open again', res => {
      this.setState({
        qrCode: true,
      });
    });
  ...
  onSuccess(e) {
    if (e.data) {
      ...
      Events.trigger('system-verify-event', result);
    }
  }
...
export default SystemVerifyWithQRCode;
